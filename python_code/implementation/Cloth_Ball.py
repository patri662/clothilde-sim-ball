...
class Cloth:
    ...
    def simulate(self, u, control):
        ...
        for s in range(self.sub_steps):
            ...
            # ball-cloth collisions
            if hasattr(self, 'ball'):
                phi = self.ballCollisions(phi)
        ...
        # check if ball class initialized to simulate it along cloth
        if hasattr(self, 'ball'):
            self.ball.simulate()
    ...
    class Ball: # give _ball name to variables to avoid identical definitions issues
        ...
        def unconstrainedStep(self, dt, g=9.8):
            ...
            self.position = self.position + dt * self.velocity # position formula
            # ceck floor collisionsfor the ball
        ...
        def floorCollisions(self):
            ...
            if self.position[2] < self.rad:  # rad because the ball surface, not center, hits the floor
                ...
                self.velocity[2] = 0         # stop vertical motion

        # method to simulate the ball that saves history as Cloth class
        def simulate(self, g=9.8):
            for s in range(self.sub_steps): # same substepping as Cloth.simulate() -> sub_steps come from Cloth class
    ...
    def ballCollisions(self, phi):
        if not hasattr(self, 'ball') or self.ball.near_cloth.shape[0] == 0:
            return phi
        ...
        diff = phi_mat[idx] - self.ball.position[np.newaxis, :]  # (k, 3)
        dists = np.sqrt(np.einsum('ij,ij->i', diff, diff)) + 1e-12  # (k,)
        normals = diff / dists[:, np.newaxis]  # (k, 3)
        
        # penetration depth: positive means inside ball
        threshold = self.ball.rad + self.rad
        penetration = threshold - dists  # (k,)
        ...
        phi_mat[idx] += (w_cloth / total_w)[:, np.newaxis] * penetration[:, np.newaxis] * normals
