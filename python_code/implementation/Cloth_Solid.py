...
class Cloth:
    ...
    # Method to visualize the simulation, including the case where solid initialized
    # if solid initialized, this method will also make the movie for the solid
    ...
    def makeMovie(self, speed = 1, repeat = True, smooth = 0):
        ...
        if self.polyscoped is False:
            self.preparePolyscope()
        # for the solid
        if hasattr(self, 'solid') and not self.solid.polyscoped:
            self.solid.preparePolyscope()
        ...
        def goThroughHistory():
            ...
            # Update Polyscope visualization for solid (as in plotMesh() function)!!
            if hasattr(self, 'solid'):
                solid_pos = self.solid.history_pos[self.ps_frame]
                ps.get_point_cloud(self.solid.label).update_point_positions(solid_pos)
                # surface mesh for solid if faces given, otherwise edges
                if self.solid.faces is not None:
                    ps.get_surface_mesh(self.solid.label + "_surface").update_vertex_positions(solid_pos)
                else:
                    ps.get_curve_network(self.solid.label + "_edges").update_node_positions(solid_pos)
            ...
            if self.ps_frame >= len(self.history_pos):
                if repeat:
                    ...
                    if hasattr(self, 'solid'):
                        # update solid position to last frame
                        ps.get_point_cloud(self.solid.label).update_point_positions(self.solid.history_pos[-1])
                        # if we have faces, draw the surface mesh, otherwise draw the edges
                        if self.solid.faces is not None:
                            ps.get_surface_mesh(self.solid.label + "_surface").update_vertex_positions(self.solid.history_pos[-1])
                        else:
                            ps.get_curve_network(self.solid.label + "_edges").update_node_positions(self.solid.history_pos[-1])
    ...
    def setSimulatorParameters(self, dt = 1/60, tol = 0.0075, sub_steps = 10,
        ...
        if hasattr(self, 'solid'): 
            self.solid.dt = self.dt
            self.solid.sub_steps = self.sub_steps
            self.solid.mu_floor = self.mu_floor
            self.solid.t_int = self.t_int # for interpolation
    ...
    class Solid: # give _ball name to variables to avoid identical definitions issues
        def __init__(self, positions, rad, mass, friction, edges = None,
                     velocities = None, name = 'solid', faces = None, kinematic = False): # name for polyscope, faces for render
            
            self.positions = np.atleast_2d(np.array(positions, dtype=float)) #current position of ball      
            self.n_nodes = self.positions.shape[0]
            self.velocities = np.zeros_like(self.positions) if velocities is None else np.atleast_2d(np.array(velocities, dtype=float))
            self.history_pos = [self.positions] #history of the vertices of the ball
            self.history_vel = [self.velocities] #history of the velocities of the ball
            ...
            self.label = name #user given name 
            self.polyscoped = False
            # quad connectivity for surface rendering, None means no surface to draw
            self.faces = None if faces is None or len(faces) == 0 else np.array(faces, dtype=int)

            # default physical parameters of the object per node
            # scalar mass = TOTAL mass of the solid, split equally across its nodes
            self.mass = np.full(self.n_nodes, mass/self.n_nodes, dtype=float) if np.isscalar(mass) else np.array(mass, dtype=float)
            self.m_inv = 1.0/self.mass
            self.mu_b = friction #friction of ball

            # definition whether is a free-solid
            # kinematic = False (default) is free solid affected by gravity and cloth collisions (bidirectional responses)
            # kinematic = True is controlled solid and not affected by gravity and one-directional collision response (solid affects cloth only)
            self.kinematic = kinematic
            self.control = np.array([], dtype=int) #indices of nodes that are controlled by the user
            self.max_step = None

            # rigid/inextensible edges (bars) between nodes (like seams treatment in cloth)
            # rest length is taken directly from the initial configuration (like Cloth.Stretch.val0)
            self.edges = np.zeros((0,2), dtype=int) if edges is None else np.array(edges, dtype=int)
            self.n_edges = self.edges.shape[0]
            if self.n_edges > 0:
                d0 = self.positions[self.edges[:,1]] - self.positions[self.edges[:,0]]
                self.rest_len = np.sqrt(np.einsum('ij,ij->i', d0, d0))
            else:
                self.rest_len = np.zeros((0,))

            # for rigid conditions, compute the centroid of the solid in its rest configuration
            w = self.mass / self.mass.sum()
            self.rest_centroid0 = (w[:, np.newaxis] * self.positions).sum(axis=0)
            self.rest_local = self.positions - self.rest_centroid0  # rest shape, centroid-relative

            # solver variables
            self.dt = None #time step for simulating
            self.tol = None #solver tolerance in %
            self.total_iters = 0 #global number of iterations perfomed when calling simulate
            self.t_int = None # substep interpolation
            self.sub_steps = None #number of substeps for simulating
            self.warning = False #for displaying the warning

            # #compute all the necesary elements for simulation only once
            # self.prepareSimulation()

            # for collisions with cloth, one entry per node
            # got from Cloth, method to query cloth vertices against ball -> computeClosePairsBall()
            self.near_cloth = [np.array([], dtype=int) for _ in range(self.n_nodes)]  # indices of close cloth vertices
            self.dists_cloth = [np.array([], dtype=float) for _ in range(self.n_nodes)]  # distances to those vertices
        ...
        def __repr__(self):
            return f"Solid(n_nodes={self.n_nodes}, n_edges={self.n_edges}, rad={self.rad}, mass={self.mass})"
        ...
        def preparePolyscope(self):
            self.polyscoped = True
            pc = ps.register_point_cloud(self.label, self.positions,
                                radius=float(np.mean(self.rad)*1.4))

            # surface mesh, same idea as Cloth's register_surface_mesh, when faces given
            if self.faces is not None:
                ps.register_surface_mesh(self.label + "_surface", self.positions, self.faces,
                                          smooth_shade=True, transparency=0.9, edge_width=0)
            else: # if not faces, draw edges as a tube network
            # create a tube connecting the nodes
                cn = ps.register_curve_network(self.label + "_edges", self.positions, self.edges)
                cn.set_radius(self.rad[0]*1.4, relative=False)

            if self.n_nodes > 1 and not np.allclose(self.rad, self.rad[0]):
                # polyscope only takes one uniform radius at registration time;
                # expose per-node radii as a scalar quantity for reference/inspection
                pc.add_scalar_quantity("radius", self.rad*1.4)
        ...
        def plotMesh(self):    
            ...
            # Plot the current mesh
            ps.get_point_cloud(self.label).update_point_positions(self.positions)
            # Plot surface mesh if faces are provided, otherwise plot edges as a tube network
            if self.faces is not None:
                ps.get_surface_mesh(self.label + "_surface").update_vertex_positions(self.positions)
            else:
                # update the tube edges connecting the nodes
                ps.get_curve_network(self.label + "_edges").update_node_positions(self.positions)
            ps.show()

        # ball's motion is free-fall, mirroring unconstrainedStep from Cloth class
        # vectorized over every node
        def unconstrainedStep(self, dt, g=9.8):
            # Euler: update velocity first, then position
            self.velocities[:,2] -= g * dt      # z-component of every node (all rows), not node[2] -> self.velocities is (n_nodes, 3)
            self.positions += dt * self.velocities # position formula

        # exact rigid-body shape enforcement: fit the single rotation+translation
        # snap every node to that exact rigid transform of the rest shape. 
        # A rigid transform can't change any internal distance/angle by construction
        def enforceRigidShape(self):
            w = self.mass / self.mass.sum()
            centroid = (w[:, np.newaxis] * self.positions).sum(axis=0)
            cur_local = self.positions - centroid
            H = (self.rest_local * w[:, np.newaxis]).T @ cur_local  # 3x3 cross-covariance
            U, S, Vt = np.linalg.svd(H)
            R = Vt.T @ U.T
            if np.linalg.det(R) < 0:  # avoid an improper rotation (reflection)
                Vt[-1, :] *= -1
                R = Vt.T @ U.T
            self.positions = centroid + (R @ self.rest_local.T).T

        # method for floor collisions, vectorized over every node
        # need to call it in unconstrainedStep after updating position to check collisions!!
        def floorCollisions(self, gamma0):
            # check which nodes touch the surface (< rad) -> cloth is 0 because is vertices/center touching floor
            # rad because the ball surface, not center, hits the floor
            ind_col = np.nonzero(self.positions[:,2] < 1.1*self.rad)[0] # save all solid's node indices that are below the floor
            if ind_col.shape[0] == 0:
                return
            # penetration depth: like ballCollisions but floor
            threshold = 1.1 * self.rad[ind_col]
            penetration = threshold - self.positions[ind_col, 2]
            
            # project to floor surface (same as phi_mat[ind_col,2] = 0)
            self.positions[ind_col, 2] = threshold
            ...
            # only for the ones hitting the floor!
            vt = gamma0[ind_col] - self.positions[ind_col]  
            vt[:,2] = 0.0   
            ...
            # friction force             
            norm_vt = np.sqrt(np.einsum('ij,ij->i', vt, vt)) # normalize tangent velocity
            quotient = (self.mu_floor*norm_Fn)/(norm_vt + 1e-12)
            k = np.minimum(1,quotient) #cannot move more than where CCD computed the intersection
            F_mu_ball_floor = k[:,np.newaxis]*vt
            # apply friction to position
            self.positions[ind_col] += F_mu_ball_floor

        # Control of the solid ONLY for not free-solid
        def processControlInputs(self, u, control):
            """
            u : (n_ctr,3) array, final target world positions for the nodes
                at control (indices into self.positions), to be reached by
                the END of this Cloth.simulate() call.
            control : list/array of node indices being driven this call.
            Returns U: is solid's control-node target for substep s, linearly interpolated from
            their CURRENT position to `u` across the substeps, as Cloth.processControlInputs.
            """
            control = np.array(control, dtype=int)
            self.control = control
            n_ctr = control.shape[0]
            if n_ctr == 0:
                return [None]*self.sub_steps
            u = np.array(u, dtype=float)
            pos0 = self.positions[control].copy()  # (n_ctr,3), current position = interpolation start
            U = [pos0 + t*(u - pos0) for t in self.t_int]
            return U

        # same as cloth's limitControlVelocity, but for the solid's control nodes
        def limitControlVelocity(self, target):
            if target is None or self.max_step is None:
                return target
            u_used = self.positions[self.control]
            du = target - u_used
            dist = np.sqrt(np.einsum('ij,ij->i', du, du))
            scale = np.minimum(1.0, self.max_step/(dist + 1e-12))
            return u_used + scale[:, np.newaxis]*du


        def applyControl(self, target):
            """
            target : (n_ctr,3) target positions for controlled nodes.
            Applies rigid transform to ALL nodes, so the
            whole solid follows its control handles and stays rigid.
            To rotate: need 4 control nodes, otherwise just translate the whole solid to the average target.
            """
            idx = self.control
            n_ctr = idx.shape[0]
            if n_ctr == 0:
                return
            if n_ctr < 4:
                delta = (target - self.positions[idx]).mean(axis=0)
                self.positions = self.positions + delta
                return
            rest_sub = self.rest_local[idx]
            w = self.mass[idx] / self.mass[idx].sum()
            centroid_target = (w[:, np.newaxis] * target).sum(axis=0)
            centroid_rest = (w[:, np.newaxis] * rest_sub).sum(axis=0)  # control subset's rest centroid
            # the control subset is symmetric about the full centroid
            rest_c = rest_sub - centroid_rest
            tgt_c = target - centroid_target
            H = (rest_c * w[:, np.newaxis]).T @ tgt_c
            U, S, Vt = np.linalg.svd(H)
            R = Vt.T @ U.T
            if np.linalg.det(R) < 0:
                Vt[-1, :] *= -1
                R = Vt.T @ U.T
            # apply to ALL nodes, measured from the control subset's rest centroid
            self.positions = centroid_target + (R @ (self.rest_local - centroid_rest).T).T

        # method to simulate the ball that saves history as Cloth class
        # kinematic = False:
        # 1) free-fall per node
        # 2) enforce edge constraints to keep solid together
        # 3) floor collisions
        # kinematic = True:
        # 1) move the solid to the target position (rigid transform)
        def simulate(self, target=None,g=9.8):
            if self.kinematic:
                target = self.limitControlVelocity(target) 
                self.applyControl(target) # already enforces rigid shape
            else:
                self.unconstrainedStep(self.dt,g)  # dt comes from Cloth class
                self.enforceRigidShape()  # rigid-body fit
            

    # method inside Cloth to add a solid by calling cloth.addSolid()
    def addSolid(self, positions, rad, mass = 1.0, friction = 0.3, edges = None, velocities = None, name = 'solid', faces = None, kinematic = False):
        return self.Solid(positions, rad, mass, friction, edges, velocities, name, faces, kinematic)

    # method to query cloth vertices against solid -> called in Cloth.selfCollisions() in Cloth.simulate(), not in Solid.simulate()!!
    ...
    def computeClosePairsBall2(self, phi_mat):
        ...
        for n, idx in enumerate(neighbors):
            idx = np.asanyarray(idx, dtype=int)
            solid.near_cloth[n] = idx

            if idx.size > 0:
                diff = phi_mat[idx] - solid.positions[n] # (k,3)
                dists = np.linalg.norm(diff, axis=1)  # (k,)
                solid.dists_cloth[n] = dists
            else:
    ...
    # loops over each solid node and resolves cloth <-> node contact independently
    @profile
    def ballCollisions(self, phi, gamma0, n_iter, max_iters=50):
        """Mass weights depend on whether solid is free (kinematic).
        A kinematic solid is infinite mass (w_ball = 0), so cloth absors
        all the correction and not bidirectional response. 
        A free solid has finite mass, so it moves back a little."""
        ...
        for n in range(solid.n_nodes):
            ...
            # vectors from that node's center to each close cloth vertex
            diff = phi_mat[idx] - solid.positions[n][np.newaxis, :]  # (k,3) pointing away from ball
            ...
            threshold = 1.1*(solid.rad[n] + self.rad)
            ...
            # mass weights: w_ball depends on kinematic or free solid. 
            w_cloth = self.m_inv[idx]               # (k,)
            w_ball  = 0.0 if solid.kinematic else 1.0 / solid.mass[n]          # scalar, same for all contacts
            ...
            # if kinematic, it is skipped completely because ball not pushed by cloth
            if not solid.kinematic:
                ball_delta = -np.mean((w_ball / total_w)[:, np.newaxis] * penetration[:, np.newaxis] * normals, axis=0)
                solid.positions[n] += ball_delta
        
            # friction here
            if solid.mu_b > 0 and n_iter < 5:
                # for the cloth
                norm_Fn = self.computeNorm(dlt_phi_idx)
                nu = dlt_phi_idx/(norm_Fn[:,np.newaxis] + 1e-12)
                v = self.positions[idx] - phi_mat[idx]
                vt = v - (self.innerProduct(v,nu)[:,np.newaxis])*nu 
                F_mu = self.frictionForce(solid.mu_b,norm_Fn,vt,cap = True)

                # for the ball
                # again, if kinematic it is skipped
                if not solid.kinematic:
                    norm_Fn = np.linalg.norm(ball_delta)
                    nu = ball_delta/(norm_Fn + 1e-12)
                    v = gamma0[n] - solid.positions[n]
                    vt = v - np.dot(v,nu)*nu 
                    norm_vt = np.linalg.norm(vt)
                    quotient = (solid.mu_b*norm_Fn)/(norm_vt + 1e-12)
                    k = np.minimum(1,quotient) #cannot move more than where CCD computed the intersection
                    F_mu_ball = k*vt
                    phi_mat[idx] += F_mu
                    solid.positions[n] += F_mu_ball

        return phi_mat.flatten(order='F')
    

    @profile
    def simulate(self, u, control, u_solid = None, control_solid = None):
        """
        u, control: cloth's own control nodes/targets, unchanged.
        u_solid, control_solid: target world positions and node
            indices for driving the solid (only meaningful if the
            solid was created with kinematic=True). Same semantics as
            u/control.
        """
        ...
        # process the solid's own control inputs, once per call (not per substep)
        if hasattr(self, 'solid') and u_solid is not None and control_solid is not None:
            U_solid = self.solid.processControlInputs(u_solid, control_solid)
        else:
            U_solid = [None]*self.sub_steps
        ...
        for s in range(self.sub_steps):
            ...
            # check if solid class initialized and get current position
            if hasattr(self, 'solid'):
                gamma0 = self.solid.positions.copy()
            ...
            # check if solid class initialized to simulate its physics along cloth
            if hasattr(self, 'solid'):
                self.solid.simulate(target=U_solid[s], g=self.g)
            ...
            if hasattr(self, 'solid'):
                self.solid.floorCollisions(gamma0)
                self.solid.velocities = (self.solid.positions - gamma0)/self.solid.dt
        ...
        # save solid state once per frame (after all substeps)
        if hasattr(self, 'solid'):
            self.solid.history_pos.append(self.solid.positions.copy())
            self.solid.history_vel.append(self.solid.velocities.copy())
