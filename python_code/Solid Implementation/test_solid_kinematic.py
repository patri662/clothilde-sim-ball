...
# template, edges = barTemplate(length=0.7, axis=1, n_segments=20) # local shape: 0.7m apart, with 20 intermediate nodes, in X axis
# positions, edges, faces = createSolidMesh([0.3,0,0.6], template, edges)  # place it at (0,0,0.6)
# clothilde.solid = clothilde.addSolid(positions, rad=0.025, mass=1, friction=0.3, edges=edges, kinematic = True) # rad = rad of each node, so reduce rad is solid nodes vert close to each other colliding
...
positions, edges, faces = createSolidMesh([0.3,0,0.8], template, edges, faces)  # place it at (0,0,0.6)
clothilde.solid = clothilde.addSolid(positions, rad=0.01, mass=1, friction=0.2, edges=edges, faces=faces, name="Cube", kinematic=True) # rad = rad of each node, so reduce rad is solid nodes vert close to each other colliding
...
# move two top corners forward and bar backward 
# to make the cloth fall and collide with the solid
inds_ctr = [0, (na*nb)-na]
tf = 200
u = X[inds_ctr]
inds_solid = [0,1]
...
# release two bottom corners and let the cloth fall onto solid
inds_ctr = []
tf = 400
...
tf = 400
u = clothilde.positions[inds_ctr]
inds_solid = [] # need 4 nodes at least to rotate solid. BUT these 4 need to be 4 corners of the same face so rotation takes the center
...
create_video_from_frames(frames_folder, "Cloth-Cube.mp4", framerate=20, cleanup=True) 